...
export default function FieldListPage() {
  ...
  // Filter state
  const [filterType, setFilterType] = useState('FIVE_A_SIDE')
  ...
  // Data state
  const [fields, setFields] = useState([])         // sub-fields of selected type
  const [slotsByField, setSlotsByField] = useState({})  // { fieldId: [slots] }
  ...
  const handleSlotClick = (slot, field) => {
  ...
  const totalAvailable = Object.values(slotsByField).flat().filter(s => s.available).length
              ...
              return (
                <div key={field.id} className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-xl bg-[#e8f9eb] flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-gray-400 font-medium">
                        {slots.filter(s => s.available).length} / {slots.length} ca trống
                      </span>
                    </div>
                  </div>
                    ...
                    {slots.map(slot => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => handleSlotClick(slot, field)}
                          className={`flex flex-col items-center justify-center p-3 rounded-2xl w-full text-center transition-all duration-200 border-2 ${
                            !isAvailable
                        ...
                        >
                          <span className={`font-bold text-base mb-0.5 ${!isAvailable ? 'opacity-50' : ''}`}>
                            {formatTime(slot.startTime)}
                          </span>
                          <span className={`text-[11px] font-semibold tracking-wider ${
                            !isAvailable ? 'opacity-50' : isSelected ? 'text-white/80' : 'text-gray-500'
                          }`}>
                            {isAvailable ? formatCurrency(slot.price) : 'Đã kín'}
                          </span>
                        </button>
                      )
                    })}
